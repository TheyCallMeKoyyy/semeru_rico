...
async function selectReturnDate(webApp, return_date) {
    test.info().annotations.push({
        type: 'allure.step',
        value: 'Select return date',
    });

    const returnDateField = webApp.locator(`//input[@id='tanggal_pulang']`); // pastikan id sesuai
     await webApp.locator(`//input[@id='is_pp']`).click(); 

    await expect(returnDateField).toBeVisible();
    await returnDateField.click();

    // Next month jika perlu, atau hilangkan jika tanggal di bulan yang sama
    await webApp.locator(`xpath=//span[@class='flatpickr-next-month']//*[name()='svg']`).nth(0).click();
    await webApp.locator(`xpath=//span[@aria-label='${return_date}']`).click();
}
