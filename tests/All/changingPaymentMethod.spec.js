...
async function changePaymentMethod(webApp, paymentMethod) {
    try {
        ...
        await webApp.locator("xpath=btn btn-radius rounded-10 btn-block bg-blue text-white").click();
    } catch (err) {
...
test('Changing payment method', async ({ webApp }) => {
    ...
    await toTicketChecking(webApp, "BBRT241011239N"); //input booking code
});
